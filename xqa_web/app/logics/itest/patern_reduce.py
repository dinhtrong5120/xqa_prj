...
import json as js
import copy
from xqa_web.app.logics.itest.sub_function import *
from xqa_web.app.logics.itest.match_unmatch_function import *
# from xqa_web.app.logics.itest.vcMaster_handle import *
...
def patern_reduce(options_car2, options_car1, link_folder_output, run_option):
    ...
    ########################################### INPUT CHECK #################################################
    err_txt = ""
    err_txt = check_input(link_folder_vc, link_master_VC, link_aabb, name_taiso, list_name_base)
    if err_txt.strip() != "": 
        return err_txt
    else:   
        ########################################### WORK FOR AABB FILE #################################################
        ...
        list_columns = df_AABB.columns.to_list()
        columns_taiso = [col for col in list_columns if (name_taiso in col)]
        columns_base = [col for col in list_columns if (list_name_base[0] in col)]

        tmp_df = df_AABB[columns_taiso + columns_base].iloc[2:]

        tmp_df['set_taiso'] = tmp_df.apply(lambda row: set(row[columns_taiso]), axis=1)
        tmp_df['set_base'] = tmp_df.apply(lambda row: set(row[columns_base]), axis=1)
        ...
        #     if tmp_df.loc[idx, 'set_taiso'] & tmp_df.loc[idx, 'set_base']:
        #         list_row_same.append(idx)
        #     else:
        #         list_row_not_same.append(idx)
        
        # print(f"list_row_same: {list_row_same}")
        # print(f"list_row_not_same: {list_row_not_same}")
