...
'''
The function checks if 'DPFC-lot' is in the DataFrame results_querry_region_7. If not, it adds a new row with empty values.
'''
def add_dpfc_lot_to_region7(df: pd.DataFrame, lots: list):
    _value = df["CADICS ID"].values
    df_len = len(df.columns)
    for lot_name in lots:
        lot_existed: bool = lot_name in _value
        if not lot_existed:
            lot_status = [None]*df_len
            lot_status[0] = lot_name
            df.loc[len(df)] = lot_status
    df = sort_status_lot_config_df(df, lots)
    return df

'''
The function is used to sort lot config to a predefined order.
'''
def sort_status_lot_config_df(df, lots):
    # Desired order for sorting
    desired_order = ["OptionCode", *lots]
    # Filter and sort the DataFrame
    sorted_df = df[df["CADICS ID"].isin(desired_order)].copy()
    sorted_df["CADICS ID"] = pd.Categorical(sorted_df["CADICS ID"], categories=desired_order, ordered=True)
    sorted_df = sorted_df.sort_values("CADICS ID")
    return sorted_df
