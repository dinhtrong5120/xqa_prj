...
@dataclass
class DpfcLot(Base):
    __tablename__ ='dpfc_lot'
    id_project = Column(Integer, primary_key=True)
    cadic_number = Column(String, primary_key=True)
    evaluate_or_not = Column(String)
    result_first = Column(String)
    report_number = Column(String)
    number_of_q_base = Column(String)
    q_base_number = Column(String)
    result_counter = Column(String)
...
def add_new_user(username, permission, project):
    engine = connect_db_tvso()
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)
    session = Session()
...
def query_data(project_name, market, powertrain, develop_case, group, lot, is_cadics=None):
    engine = connect_db_tvso()
    ...
    if project is not None:
        ...
        if is_cadics:
            temp_df = create_dpfc_df_from_query(id_project)
            data_df, result_df = add_dpfc_df_to_data_df(data_df, result_df, temp_df)
...
'''
The function is used to query dfpc_lot
'''
def create_dpfc_df_from_query(id_project: int):
    engine = connect_db_tvso()
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)
    session = Session()
    query =  session.query(DpfcLot).filter(DpfcLot.id_project==id_project)
    df =  pd.read_sql_query(sql=query.statement, con=session.bind)
    return df

'''
The function is used to add dpfc data to main dataframe
parameter:
    data_df: dataframe of main_table
    result_df: dataframe of header
    temp_df: dataframe of dpfc
'''
def add_dpfc_df_to_data_df(data_df, result_df, temp_df):
    # query rows of dpfc table. Use to create CADICS dataframe
    swap_columns = ["evaluate_or_not", "result_first", "report_number", "number_of_q_base",
        "q_base_number", "result_counter", "comment"]
    
    if "id_project" in temp_df.columns:
        temp_df.drop("id_project", axis=1, inplace=True)

    # Join main_table dataframe with DPFC dataframe
    data_df = data_df.join(temp_df.set_index("cadic_number"), on="cadic_number", how="left")
    # result_df = _add_dpfc_header_to_header(result_df, swap_columns)
    _header_df = copy_header(result_df, swap_columns, "col61", "col67", "DPFC")
    result_df = pd.concat([result_df, _header_df], axis=1)

    data_df = move_columns(data_df, cols_to_move=swap_columns, new_index=60)
    result_df = move_columns(result_df, cols_to_move=swap_columns, new_index=60)
...
'''
The function is used to create a copy of dataframe from colA to colB, then add head for copy df
'''
def copy_header(df: pd.DataFrame, headers: list, colA: str, colB: str, lot_name: str) -> pd.DataFrame:
    _head = df.loc[:, colA: colB].copy()
    _head.loc[2, colA] = lot_name
    _head.columns = headers
    return _head

'''
The function is used to update dfpc_lot
'''
def update_dpfc_lot(data: dict):
    try:
        cadic_number, id_project, evaluate_or_not, result_first, report_number, number_of_q_base, q_base_number, result_counter, comment = data
        engine = connect_db_tvso()
        Base.metadata.create_all(engine)
        Session = sessionmaker(bind=engine)
        session = Session()
        dpfc_lot = DpfcLot(cadic_number, id_project, evaluate_or_not, result_first, report_number, number_of_q_base, q_base_number, result_counter, comment)
        session.bulk_save_objects(dpfc_lot)
        session.commit()
    except Exception as e:
        print(e)
        session.rollback()