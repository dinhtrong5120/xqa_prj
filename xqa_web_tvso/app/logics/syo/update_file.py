...
from xqa_web_tvso.app.setting import BASE_DIR_DATAS, BASE_DIR_CADICS_TEMP
...
# def extract(file_path, output_dir):
#     # Reasoning
#     # 1. Check file type (.zip or .7z)
#     # 2. For .zip: use Python zipfile (cross-platform, no external tool needed)
#     # 3. For .7z: find 7z executable (Windows: 7z.exe, Linux: 7z), error if not found

#     # Ensure output directory exists
#     if not os.path.exists(output_dir):
#         os.makedirs(output_dir)

#     # .zip extraction (cross-platform)
#     if file_path.lower().endswith(".zip"):
#         try:
#             with zipfile.ZipFile(file_path, 'r') as zip_ref:
#                 zip_ref.extractall(output_dir)
#             folder = file_path[:-4]
#             move_all_items(folder, output_dir) # Only needed if zip creates a subfolder
#             delete_folder(folder)
...
#             seven_zip_path = shutil.which("7z")
#             if seven_zip_path is None:
#                 # Common install location for 7-Zip
#                 default_path = r"C:\Program Files\7-Zip\7z.exe"
#                 if os.path.exists(default_path):
#                     seven_zip_path = default_path
#                 else:
#                     raise FileNotFoundError(
#                         "7z.exe not found. Please install 7-Zip and add it to your PATH."
#                     )
#             cmd = [seven_zip_path, "x", file_path, f"-o{output_dir}", "-y"]
#         else:
#             # Assume Linux/Mac, try '7z'
#             seven_zip_path = shutil.which("7z")
#             if seven_zip_path is None:
#                 raise FileNotFoundError(
#                     "'7z' command not found. Please install p7zip-full (sudo apt install p7zip-full)"
#                 )
#             cmd = [seven_zip_path, "x", file_path, f"-o{output_dir}", "-y"]
...
#             raise RuntimeError(f"7z extraction failed with exit code {result.returncode}")

#         folder = file_path[:-3]
#         move_all_items(folder, output_dir) # Only needed if 7z creates a subfolder
#         delete_folder(folder)

#     else:
#         raise ValueError("Unsupported file type for extraction: only .zip and .7z supported.")
