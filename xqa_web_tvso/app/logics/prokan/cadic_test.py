...
from xqa_web_tvso.app.logics.syo.db.funtion_database import update_new, get_header
...
def create_cadics_new(case, market, powertrain, car, list_group):
    ...
    if not os.path.exists(folder_out_check):
...
def get_infor_car(df_spec):
    ...
    dict_optioncode = {}
    # working = os.path.dirname(__file__)
    # link_form_cadic = "./xqa_web_tvso/app/form_out/form_cadics.csv"
...
def get_group_karenhyo12(folder_data, car, folder_out, list_group, market, case, powertrain):
    ...
    if not os.path.exists(link_file_spec):
        link_file_spec = None
    # =============================================================================================
    list_file = []
