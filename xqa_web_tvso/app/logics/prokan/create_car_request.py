...
def create_car_request(data_cadics, data_kanrenhyo, car_number, lot, start_table3, end_table3, group):
    ...
    for col in range(start_table3, end_table3):
        ...
        can_harness = data_kanrenhyo.iat[27, col]
        body_color_required = data_kanrenhyo.iat[28, col]
        body_color_not_required = data_kanrenhyo.iat[29, col]
        interior_color_required = data_kanrenhyo.iat[30, col]
        interior_color_not_required = data_kanrenhyo.iat[31, col]
        ...
        if sum_time > 0:
            ...
            for row in range(len(data_frame)):
                ...
                dict_data[140] = body_color_required
                dict_data[141] = body_color_not_required
                dict_data[142] = interior_color_required
                dict_data[143] = interior_color_not_required
                # dict_data[144] = can_harness # CANハーネス | w、w/o、(空白:どちらでも可)
                dict_data[145] = can_harness # CANハーネス | w、w/o、(空白:どちらでも可)
...
def selected_car(data_cadics, list_cadics, car_number, col_comment):
    ...
    if len(data_frame) != 0:
        ...
        result = data_frame[[col_comment,*range(start_index, start_index + car_number)]]
        result.reset_index(drop=True)
        result=result.drop_duplicates()
        result = result.reset_index(drop=True)
        frame_pick_car=result[[*range(start_index, start_index + car_number)]]
