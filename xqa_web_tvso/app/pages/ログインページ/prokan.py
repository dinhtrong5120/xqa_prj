...
from xqa_web_tvso.app.logics.syo.db.function_database_new import update_syo
from xqa_web_tvso.app.logics.syo.db.funtion_database import query_data, update_edit
# from xqa_web_tvso.app.logics.syo.update_file import get_csrf_token, update_file_into_server_new, update_file_after_edit
from xqa_web_tvso.app.logics.prokan.read_data_view import check_file_out, read_output, write_cadic_temp, zip_folder
from xqa_web_tvso.app.logics.prokan.cadic_test import create_cadics_new
from xqa_web_tvso.app.logics.prokan.create_document_step2 import create_doc
from xqa_web_tvso.app.logics.prokan.classify_group import *
from xqa_web_tvso.app.logics.prokan.update_file import get_csrf_token, update_file_into_server_new, update_file_after_edit, process_uploaded_車両仕様表_Form_files
from xqa_web_tvso.app.logics.prokan.step3_create_output import fill_final_output, ensure_latest_form_in_output
from xqa_web_tvso.app.logics.prokan.step3_check_form import check_all_sheet_formats
from xqa_web_tvso.app.setting import BASE_DIR_DATAS, BASE_DIR_FORM_OUT
...
def main():
            ...
            if modal_create_cadics.is_open():
                with modal_create_cadics.container():
                    with st.spinner(text="In progress..."):
                        ...
                        _, data, _, _ = query_data(str(st.session_state.code_prokan).upper(),
                                                        st.session_state.plant_prokan,
                                                        st.session_state.pwt_prokan,
                                                        st.session_state.case_prokan,
                                                        st.session_state.dev_prokan,
                                                        # 'ALL')
                                                        st.session_state.plant2_prokan, is_cadics=True)
                        ...
                        if st.session_state.plant2_prokan in ["ALL", "DPFC", "DS", "DC", "PFC", "VC", "PT1", "PT2"]:
    ...
    with col2_1:
        ...
        if option_view_prokan == "CADICS項目":
            st.session_state.list_sheet = ["ALL", "DPFC", "DS", "DC", "PFC", "VC", "PT1", "PT2"]
            ...
            if st.session_state.code_prokan and st.session_state.code_prokan.strip():
                ...
                if not os.path.exists(file_form):
                    ...
                    file_form = "xqa_web_tvso/app/form_out/Step2/車両仕様表_Form.xlsx"
            else:
                file_form = "xqa_web_tvso/app/form_out/Step2/車両仕様表_Form.xlsx"
